...
namespace Zork.Common
{
    public class Game
    {
        ...
        private void Take(string itemName)
        {
            ...
            {
                if (itemToTake.Type == "Enemy")
                {
                    Console.WriteLine($"You can't take a {itemToTake}!");
                }
                else
                {
        ...
        private void Attack(string itemName)
        {
            Item itemToAttack = Player.CurrentRoom.Inventory.FirstOrDefault(item => string.Compare(item.Name, itemName, ignoreCase: true) == 0);
            ...
            {
                if (itemToAttack.Type == "Enemy")
                {

                    Console.WriteLine($"You attacked {itemToAttack}");
                    if (itemToAttack.Health == 50)
                    {
                        itemToAttack.Health = itemToAttack.Health - 25;
                        Console.WriteLine($"{itemToAttack} health is at {itemToAttack.Health}");
                    }
                    else
                    {
                        Console.WriteLine($"{itemToAttack} has been slain!");
                    }
                }
                else
                {
